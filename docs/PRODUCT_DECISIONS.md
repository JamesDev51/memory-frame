...
## V2 (2026-09-07) — 작업 보존과 기본 편집 강화

- 원본과 칸별 인스턴스를 분리하고 사진 중복 사용을 허용한다. 슬롯 교환 시 크롭도 따라간다.
- 업로드 후 자동 배치는 하지 않는다. 명시적 빈칸 채우기만 추가한다.
- 컷 수 감소는 숨김이며 배치 삭제가 아니다. 모양별 상태를 보관한다.
- 서버 없는 제품 원칙 유지: IndexedDB 원본 저장 + 파일 백업. 기기간 자동 동기화나 계정 저장은 제공하지 않는다.
- 내 작업 삭제는 확인 후 영구 삭제; 칸 제거와 원본 삭제는 실행 취소 가능.
- Dearly의 작은 고정 카드 배치를 복사하지 않고 최대 크기 배치를 유지한다. 카드 비율·옅은 회색 종이·짧은 그림자를 반영한다.
- 복잡한 카드 옵션은 상세 설정에 넣고 추천 스타일 일괄 적용 버튼은 복원하지 않는다.
