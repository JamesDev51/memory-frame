...
추천 스타일 버튼을 삭제한다. 오른쪽 메뉴는 배치/사진 개수→사진 스타일→종이 배경/여백→간격/그림자→액자/인쇄→공간 미리보기→저장 순서다. 사진 개수는 항상 노출하며 기존 프리셋을 즉시 변경할 수 있다. 기본 사진 외에 흰 테두리와 넓은 하단 여백을 가진 폴라로이드 옵션을 추가한다. 카드와 실제 사진 창의 좌표를 분리해 편집/DPI/출력에 동일하게 적용한다.

빈칸에서도 그림자 강도를 확인할 수 있도록 편집 미리보기에만 빈 종이 카드를 렌더링한다. 내보내기 및 공간 미리보기에는 실제 배치 사진만 표시한다. 그림자 강도와 간격은 독립이며 간격0에서는 이웃 사진이 그림자를 가릴 수 있다. 좁은 사이드바의 2열 간격/그림자 제어를 1열로 바꿔 줄바꿈을 줄였다.

## V2 — Dearly 확장 벤치마크 반영 (2026-09-07)

사용자가 전체 구현·GitHub 반영·Vercel 배포·검수를 승인했다. 최신 요구는 기존 V1 제한보다 우선한다.

- 시작 화면은 빈 그리드 편집 화면. 사진 추가는 보관함만 채운다. 사용자가 누르는 ‘미사용 사진으로 빈칸 채우기’만 예외적으로 일괄 배치한다.
- 원본 PhotoItem과 Placement 인스턴스를 분리했다. 같은 원본을 중복 배치하고 각 칸의 크롭/회전을 독립 편집한다. 보관함 drop은 복제/교체, 슬롯 drop은 편집 상태를 포함한 교환이다.
- 사진 개수를 줄여도 placements를 자르지 않는다. 표시·출력·사용 상태는 활성 칸만 기준으로 한다. 그리드/하트는 각자의 마지막 배치·개수를 기억한다. 처음 다른 모양으로 바꾸면 빈 배치로 시작하고 되돌아오면 기존 배치를 복원한다.
- IndexedDB에 원본 File과 전체 snapshot을 저장한다. 자동 저장은 700ms 지연, 작업 전환 전 flush, 실패 표시, 브라우저 종료 시 미저장 경고를 제공한다. 저장/삭제 작업 중 동시 조작을 막는다. 삭제한 마지막 사진도 빈 작업으로 저장된다.
- 내 작업: 이름 변경, 썸네일, 시각, 이어서 편집, 복제, 삭제. 내보내기는 같은 작업을 갱신한다. `.memoryframe` 파일은 원본 포함 JSON 백업이며 다른 기기에서 가져올 수 있다. 계정/서버 동기화 없음. 브라우저 데이터 삭제 시 로컬 작업도 삭제되므로 파일 백업 안내.
- 90도 좌우 회전, 최대3배 확대, 제스처 단위 실행 취소. 선택된 칸 드래그는 크롭 이동, 편집을 닫으면 슬롯 드래그 교환. 오른쪽 인라인 편집 패널로 중앙 용지 크기 유지.
- 폴라로이드 기본 H/W=1.1628, 좌우/위4.82%, 아래21.1%. 카드 #f7f7f7 또는 흰색. 상세 테두리2~10%, 아래10~30%. 그림자 blur=.723% W, y=.602% W, x=0; 슬라이더는 alpha 0~.55, 기본OFF.
- 그리드 폴라로이드는 실제 카드 경계 사이 X/Y 간격이 같고 중앙 정렬된다. 비율을 유지하면서 용지에 가능한 최대 크기로 채우기 때문에 용지/행열 비율이 다르면 상하/좌우 바깥 여백은 달라질 수 있다.
- JPG 추가. PNG/JPG/PDF 모두 공통 렌더링. mm·픽셀·DPI 표시. 사진 회전은 미리보기/출력/DPI에 공통 반영.
- 검증 범위와 남은 제한은 docs/VALIDATION.md의 V2 기록 확인. 브라우저 직접 검수 또는 실물 인쇄를 자동 테스트와 혼동하지 말 것.
