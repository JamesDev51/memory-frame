...
## V2 구현 범위 (2026-09-07 최신 요구)

1. 편집 화면 직접 시작, 여러 사진 보관함 추가, 선택형 빈칸 채우기.
2. 독립적인 칸별 확대/크롭/90도 회전, 중복 사용 횟수와 활성 칸 번호, 교환·제거·실행 취소.
3. 컷 수 감소/모양 전환 시 배치 보존.
4. 브라우저 자동 저장, 내 작업 목록·이름·복제·삭제·복원, 원본 포함 작업 파일 백업/가져오기.
5. 실제 카드 간격이 균일한 폴라로이드, 카드색과 상세 여백, 밀착 그림자 슬라이더.
6. JPG/PNG/PDF, mm·픽셀·DPI 확인 및 출력 안전 여백.
7. 노트북 중앙 용지와 좌측 사진관리·우측 설정을 함께 표시. 오른쪽 사진 편집은 모달 없이 제공.

수용 조건: 같은 사진의 다른 칸 편집은 독립적; 20→9→20과 모양 전환 복원; 저장 실패는 성공으로 표시하지 않음; 마지막 원본 삭제도 저장; 출력에서 빈칸과 편집 UI 제외; 용지 실측과 회전/그림자 일치. 실제 인쇄 및 기기간 자동 복원은 검증/제공 범위 밖이다.
