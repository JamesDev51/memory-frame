...
- 사진별 독립 크롭/회전, 중복 배치, 선택형 빈칸 채우기
- 컷 수·모양 변경 후 배치 복원
- 브라우저 자동 저장과 내 작업, 원본 포함 `.memoryframe` 백업/가져오기
- 폴라로이드 세부 비율·카드색·밀착 그림자, JPG/PNG/PDF

작업은 현재 브라우저에 보관됩니다. 브라우저 데이터 삭제 또는 다른 기기 이동에 대비해 ‘내 작업 → 현재 작업 파일 백업’을 이용하세요. 사용자 사진은 서버로 업로드하지 않습니다.
